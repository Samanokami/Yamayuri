...
#alter_arr =[ [temp.csv,temp2.csv,temp3.csv],[temp4.csv,temp5.csv,temp6.csv]]
function run_commands(command,alter_arr){
	system(command)
	close(command)
	for(item1=1;item1<=length(alter_arr);item1++){
		for(item2=1;item2<=length(alter_arr[item1]);item2++){
			gsub(alter_arr[item1][item2],alter_arr[item1+1][item2],command)
		}
		system(command)
		close(command)
	}
...
BEGIN{
	...
	command="nkf -wLu " ARGV[1] " > " copy_filename "_uni.txt"	#文字コード変換
	system(command)
	close(command)
...
#	command="nkf -sLw temp.csv > temp2.csv"	#処理対象ファイルの文字コードをSJISに変更
#	system(command)
#	close(command)
#	
#	command="nkf -sLw temp3.csv > temp4.csv"	#処理対象ファイルの文字コードをSJISに変更
#	system(command)
#	close(command)
	
	command="cp temp.csv temp2.csv"	#処理対象ファイルのコピー
	system(command)
	close(command)

	command="cp temp3.csv temp4.csv"	#処理対象ファイルのコピー
	system(command)
	close(command)
	...
	command = "ls html/levenshtein"	#フォルダが存在するか確認
	while(command|getline){
		files = files "," $0
	}
	sub(/,/,"",files)
	if(files!~output_filename){	#フォルダが存在しない場合のみ、フォルダを新規作成
		command="mkdir ./html/levenshtein/" output_filename			
		system(command)
		close(command)
	}
	
	command = "ls html/jarowinkler"	#フォルダが存在するか確認
	while(command|getline){
		files = files "," $0
	}
	sub(/,/,"",files)
...
#	command="nkf -sLw copy.r > copy2.r"	#Rスクリプトの文字コードを変更
#	system(command)
#	close(command)
#
#	command="nkf -sLw copy3.r > copy4.r"	#Rスクリプトの文字コードを変更
#	system(command)
#	close(command)

	command="cp copy.r copy2.r"	#Rスクリプトのコピー
	system(command)
	close(command)

	command="cp copy3.r copy4.r"	#Rスクリプトのコピー
	system(command)
	close(command)

	command = "r --vanilla --slave < copy2.r"	#Rスクリプトを実行
	system(command)
	close(command)

	command = "r --vanilla --slave < copy4.r"	#Rスクリプトを実行
	system(command)
	close(command)
...
#	close(command)
	...
	command="cp ./html/levenshtein/" output_filename "/" output_filename ".html ./html/levenshtein/" output_filename "/" output_filename "_2.html"	#html出力ファイルのコピー
	system(command)
	close(command)

	command="cp ./html/jarowinkler/" output_filename "/" output_filename ".html ./html/jarowinkler/" output_filename "/" output_filename "_2.html"	#html出力ファイルのコピー
	system(command)
	close(command)
	...
	command="nkf -sLw ./html/levenshtein/" output_filename "/" output_filename "_" target ".html > ./html/levenshtein/" output_filename "/" output_filename ".html"	#ファイル名の連番を削除
	system(command)
	close(command)
	
	command="nkf -sLw ./html/jarowinkler/" output_filename "/" output_filename "_" target ".html > ./html/jarowinkler/" output_filename "/" output_filename ".html"	#ファイル名の連番を削除
	system(command)
	close(command)
...
#	close(command)
	...
	command="nkf -sLw ./html/index2.html > ./html/index.html"	#最後にhtmlファイルの文字コードを変更
	system(command)
	close(command)
	
#	command="cp ./html/index2.html ./html/index.html"	#最後にhtmlファイルの文字コードを変更
#	system(command)
#	close(command)
