...
function conv1(){#処理前の結果書き込み用データを一時フォルダに変換出力
	...
	gsub("./temp/","./save/",copy)
	command = "nkf -wLu " copy " > " output_file_name
	system(command)
	close(command)
}
function conv2(){#テキストファイルを一時フォルダに変換出力
	for(item=1;item<=ARGC-1;item++){
		...
		command = "nkf -wLu " ARGV[item] " > ./temp/" copy_file[item]
		system(command)
		close(command)
	}
}
...
function conv4(){#処理結果を一時フォルダから保存フォルダに変換出力
	...
	gsub("./temp/","",copy)
	if(answer2==2){
		command = "nkf -wLu " output_file_name " >> ./save/" copy
	}else if(answer2==3){
		command = "nkf -wLu " output_file_name " > ./save/" copy
	}
	system(command)
	...
	if(answer2==2||answer2==3){
		command = "nkf -wLu " output_file_name " >> ./backup/" copy 
	}
	system(command)
	close(command)
